CLASS zcl_renting_load_data_2533 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_RENTING_LOAD_DATA_2533 IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.

    DATA: lt_brands    TYPE TABLE OF zrent_brand_2533,
          lt_cars      TYPE TABLE OF zrent_cars_2533,
          lt_customers TYPE TABLE OF zrent_cust_2533.
...
** BRANDS **
    ...
    DELETE FROM zrent_brand_2533.
    INSERT zrent_brand_2533 FROM TABLE @lt_brands.
    SELECT * FROM zrent_brand_2533 INTO TABLE
    @lt_brands.
    out->write( sy-dbcnt ).
...
** CARS**
    lt_cars = VALUE #( ( matricula = '1246GSA'
    marca = 'BMW' modelo = 'SERIE 4' color = 'BLANCO'
    motor = '2.5' potencia
    = '210' und_potencia = 'CV'
    ...
    DELETE FROM zrent_cars_2533.
    INSERT zrent_cars_2533 FROM TABLE @lt_cars.
    SELECT * FROM zrent_cars_2533 INTO TABLE @lt_cars.
    out->write( sy-dbcnt ).

    out->write( 'zrent_cars_2533 data inserted successfully!' ).
...
** CUSTOMERS**
    ...
    delete from zrent_cust_2533.
    INSERT zrent_cust_2533 FROM TABLE @lt_customers.
    SELECT * FROM zrent_cust_2533 INTO TABLE
    @lt_customers.
    out->write( sy-dbcnt ).
    out->write( 'zrent_cust_2533 data inserted successfully!' ).
