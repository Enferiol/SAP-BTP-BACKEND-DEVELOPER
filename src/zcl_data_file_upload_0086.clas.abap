CLASS zcl_data_file_upload_0086 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    ...
    METHODS: get_imput_field_value IMPORTING name TYPE string
    dataref TYPE data RETURNING VALUE(value) TYPE string.
...
CLASS zcl_data_file_upload_0086 IMPLEMENTATION.
  ...
  METHOD if_http_service_extension~handle_request.
    ...
    CASE request->get_method( ).
      ...
      WHEN CONV string( if_web_http_client=>get ).
        ...
        DATA(sap_taable_request) = request->get_header_field( 'sap-table-request' ).
        IF sap_taable_request IS INITIAL.
          response->set_text( get_html( ) ).
        ENDIF.
*        ELSE.
*          DATA(name_filter) = xco_cp_abap_repository=>object_name->get_filter( xco_cp_abap_sql=>constraint->contains_pattern( to_upper( sap_taable_request ) && '%' )  ).
*          DATA(objects) = xco_cp_abap_repository=>objects->tabl->where( VALUE #( ( name_filter ) ) )->in( xco_cp_abap=>repository )->get( ).
*          DATA(res) = '['.
...
* Unpack input field values such as tablename, dataoption, etc.
        ...
        IF ui_dataref IS BOUND.
          ...
          tablename = me->get_imput_field_value( name = `TABLENAME` dataref = <ui_dataref> ).
          dataoption = me->get_imput_field_value( name = `DATAOPTION` dataref = <ui_dataref> ).
